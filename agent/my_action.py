...
class TeleportPointAction(CustomAction):
    ...
    def run(
        ...
        try:
            ...
            # 4. OCR 搜索地图名字并点击
            img = context.tasker.controller.post_screencap().wait().get()
            ocr_result: RecognitionDetail = context.run_recognition(
                "通用文字识别",
                img,
                pipeline_override={
            ...
            rect = Rect(*item.box)
            logger.info(f"dest_map: {rect}, {item.text}")
            ...
            if not is_direct_tp.hit:
                ...
                for item in ocr_result.all_results:
                    score = fuzz.ratio(item.text, alias)
                    item.score = score
                # 重新根据匹配分数排序
                sorted_items = sorted(
                    ocr_result.all_results, key=lambda obj: obj.score, reverse=True
                )
                item = sorted_items[0]
                rect = Rect(*item.box)
                logger.info(f"dest_tele_point: {rect}, {item.text}")
                point_x = int(rect.x + rect.w / 2)
                point_y = int(rect.y + rect.h / 2)
                # 选择传送点
                context.tasker.controller.post_click(point_x, point_y)
...
class AutoFishingAction(CustomAction):
    ...
    def __init__(self):
        ...
        # 定义触控通道常量
        self.REEL_IN_CONTACT: int = 0
        self.BOWING_CONTACT: int = 1
    ...
    def run(
        ...
        try:
            ...
            # 启动收线
            result = self.startReelIn(context)
            if not result:
                logger.error("收线失败")
                return False
    ...
    def _reelLoop(self, context: Context) -> bool:
        ...
        """
        # 截图
        screencap_result = context.tasker.controller.post_screencap().wait()
        img: numpy.ndarray = screencap_result.get()

        # 获取方向
        bow_direction = self.getBowDirection(context, img)
        self.updateBowState(context, bow_direction)

        # 检查是否还在收线
        is_reeling = self.checkIfReeling(context, img)
        if not is_reeling:
    ...
    def updateBowState(self, context: Context, bow_direction: str | None):
        """
        根据检测到的箭头方向更新拉弓方向状态
        """
        ...
        if bow_direction:
            ...
            if self._currentBowDirection is None and \
               (current_time - self._lastBowDirectionChangeTime) > 1:
                ...
                if not self.startBow(context, bow_direction):
                    logger.error("拉弓失败！")
                    return
